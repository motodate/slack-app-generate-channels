...
- [x] 追加: `app/application/user_resolver_service.py`
  - [x] `UserResolverService.resolve(email_list: EmailAddressList)` を実装（Facade注入）。
  - [x] 既存 `resolve_users(slack_client, email_list)` はラッパーとして残し、内部でサービスを呼ぶ。
...
- [x] テスト: サービス単体のモックテストを追加（既存UIテストは不変）。
...
2. PR1/PR2/PR3/PR4 はマージ済み。互換性維持を再確認（全テストGREEN）。
3. PR5の着手: 統合・クリーンアップ（インスタンス再利用、型注釈強化、`__signature__`撤廃、VO等値性、正規化仕様確認）。
4. 併行タスク（任意）: 補助PR aux8〜aux12 等で型/エラーUX/定数集約/persistenceを小さく順次対応。
